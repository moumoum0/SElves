...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_12_13 = object : Migration(12, 13) {
            override fun migrate(database: SupportSQLiteDatabase) {
                try {
                    database.execSQL("ALTER TABLE members ADD COLUMN bio TEXT NOT NULL DEFAULT ''")
                    Log.d("AppDatabase", "数据库迁移 12->13 完成：members表已添加bio字段")
                } catch (e: Exception) {
                    Log.e("AppDatabase", "数据库迁移 12->13 失败: ${e.message}", e)
                    throw e
                }
            }
        }
        ...
        private val MIGRATION_14_15 = object : Migration(14, 15) {
            override fun migrate(database: SupportSQLiteDatabase) {
                try {
                    database.execSQL("ALTER TABLE systems ADD COLUMN description TEXT NOT NULL DEFAULT ''")
                    Log.d("AppDatabase", "数据库迁移 14->15 完成：systems表已添加description字段")
                } catch (e: Exception) {
                    Log.e("AppDatabase", "数据库迁移 14->15 失败: ${e.message}", e)
                    throw e
                }
            }
        }
        ...
        private val MIGRATION_15_16 = object : Migration(15, 16) {
            override fun migrate(database: SupportSQLiteDatabase) {
                try {
                    database.execSQL("ALTER TABLE members ADD COLUMN groups TEXT NOT NULL DEFAULT ''")
                    Log.d("AppDatabase", "数据库迁移 15->16 完成：members表已添加groups字段")
                } catch (e: Exception) {
                    Log.e("AppDatabase", "数据库迁移 15->16 失败: ${e.message}", e)
                    throw e
                }
            }
        }
        ...
        private val MIGRATION_17_18 = object : Migration(17, 18) {
            override fun migrate(database: SupportSQLiteDatabase) {
                try {
                    database.execSQL("ALTER TABLE member_groups ADD COLUMN parentName TEXT")
                    Log.d("AppDatabase", "数据库迁移 17->18 完成：member_groups表已添加parentName字段")
                } catch (e: Exception) {
                    Log.e("AppDatabase", "数据库迁移 17->18 失败: ${e.message}", e)
                    throw e
                }
            }
        }
        ...
        /**
         * 成员增加 isAdmin。
         * 默认全部非管理员；启动后若存在成员但无管理员，由 UI 引导用户勾选。
         */
        private val MIGRATION_19_20 = object : Migration(19, 20) {
            override fun migrate(database: SupportSQLiteDatabase) {
                try {
                    database.execSQL(
                        "ALTER TABLE members ADD COLUMN isAdmin INTEGER NOT NULL DEFAULT 0"
                    )
                    Log.d("AppDatabase", "数据库迁移 19->20 完成：members.isAdmin 已添加（默认非管理员）")
                } catch (e: Exception) {
                    Log.e("AppDatabase", "数据库迁移 19->20 失败: ${e.message}", e)
                    throw e
                }
            }
        }
